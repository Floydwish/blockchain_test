...
contract myBank{
    ...
    function getTop3Depositors(uint index) public view returns (address){
        require(index <top3Depositors.length, "Index out of bounds");
        return top3Depositors[index];
    }
    ...
    function updateTop3Depositors() internal{
        ...
        for(uint i = 0; i < top3Depositors.length; i++){
            if(top3Depositors[i] == currentUser){
                isInTop3 = true;
                break;
            }
        }

        if(!isInTop3){      // 如果当前用户不在Top3中，则需要更新Top3；否则不更新地址（后面会排序）
